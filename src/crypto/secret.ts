...
import { HASH_NAME, HMAC_NAME } from "../const";
...
export class Secret {
   ...
   * @param {ArrayBuffer} iv - 12 bytes for AES-GCM
   * @param {ArrayBuffer} [aad] - Optional associated data for authentication
   * @returns
   *
   * @memberOf Secret
   */
  public static encrypt(
    ...
    if (aad) {
      algorithm.additionalData = new Uint8Array(aad);
    }

    return getEngine().crypto.subtle.encrypt(algorithm, key, data);
  }
  ...
  public static decrypt(
    ...
    return getEngine().crypto.subtle.decrypt(algorithm, key, data);
  }
   ...
   * @memberOf Secret
   */
  public static importHMAC(raw: ArrayBuffer, debug: boolean = false) {
   ...
   * @memberOf Secret
   */
  public static importAES(raw: ArrayBuffer, debug: boolean = false) {
    return getEngine().crypto.subtle.importKey(
      "raw",
      raw,
      AES_ALGORITHM,
  ...
  public static async HKDF(
    ...
    for (let i = 0; i < keysCount; i++) {
      T[i + 1] = await this.sign(
        PRK,
        combine(T[i], info, new Uint8Array([i + 1]).buffer as ArrayBuffer)
      );
    }
    return T.slice(1);
  }
